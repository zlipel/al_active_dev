...
# Route SLURM logs to the mode/policy subfolder.
LENGTH_DIR="PATHS_FIXED_LENGTH"
[[ "$LENGTH_CHANGES" == true ]] && LENGTH_DIR="PATHS"
LOG_DEST="${SCRATCH_AL}/${LENGTH_DIR}/${MODEL}/${MODE^^}/${POLICY}/logs"
mkdir -p "$LOG_DEST"
SLURM_OUT="${SLURM_SUBMIT_DIR:-.}/run_beams.out"
SLURM_ERR="${SLURM_SUBMIT_DIR:-.}/run_beams.err"
[[ -f "$SLURM_OUT" ]] && mv "$SLURM_OUT" "$LOG_DEST/run_beams_${SLURM_JOB_ID:-local}.out"
[[ -f "$SLURM_ERR" ]] && mv "$SLURM_ERR" "$LOG_DEST/run_beams_${SLURM_JOB_ID:-local}.err"
